...
export const SelectCategory: React.FC<Props> = ({ tournamentId, onChange, defaultSelected = 0, maxSubscription }) => {
  ...
  const [paidCount, userHasSub] = useMemo(() => {
    const hasSubsIn = categories.find(c => !!c?.subscriptions?.length && !!c?.subscriptions?.find(f => f.userId === userData?.id))
    const count =
      hasSubsIn &&
        ...
        {categories?.length ? (
          <>
            {categories.map(category => {
              ...
              const hasSub = !!category?.subscriptions?.find(f => f.userId === userData?.id)
